package com.shulian.safe.drm.framework.apilog.core.interceptor;
...
import com.shulian.safe.drm.framework.common.util.spring.SpringUtils;
...
public class ApiAccessLogInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) {
        ...
        if (!SpringUtils.isProd()) {
            ...
            if (CollUtil.isEmpty(queryString) && StrUtil.isEmpty(requestBody)) {
                log.info("[preHandle][requestId({})][开始请求 URL({}) 无参数]",
                        RequestIdContext.getRequestId(),
                        requestURI);
            } else {
                log.info("[preHandle][requestId({})][开始请求 URL({}) 参数({})]",
                        RequestIdContext.getRequestId(),
                        requestURI,
    ...
    public void afterCompletion(HttpServletRequest request, HttpServletResponse response, Object handler, Exception ex) {
        ...
        if (!SpringUtils.isProd()) {
            ...
            stopWatch.stop();
            log.info("[afterCompletion][requestId({})][完成请求 URL({}) 耗时({} ms)]",
                    RequestIdContext.getRequestId(),
