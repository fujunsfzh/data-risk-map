package com.shulian.safe.drm.framework.web.core.handler;

import cn.hutool.core.exceptions.ExceptionUtil;
import cn.hutool.core.map.MapUtil;
import cn.hutool.core.util.ObjUtil;
import com.shulian.safe.drm.framework.apilog.core.service.ApiErrorLogFrameworkService;
...
import com.shulian.safe.drm.framework.common.util.collection.SetUtils;
...
import static com.shulian.safe.drm.framework.common.exception.enums.GlobalErrorCodeConstants.*;
...
public class GlobalExceptionHandler {
    ...
    public CommonResult<?> IllegalArgumentExceptionHandler(HttpServletRequest req, IllegalArgumentException ex) {
        ...
        log.error("[IllegalArgumentExceptionHandler]:{}", ex.getMessage(), ex);
    ...
    public CommonResult<?> defaultExceptionHandler(HttpServletRequest req, Throwable ex) {
        ...
        // 情况二：处理异常
        log.error("[requestId:{}][defaultExceptionHandler]", requestId, ex);
        // 插入异常日志
    ...
    private void buildExceptionLog(ApiErrorLogCreateReqDTO errorLog, HttpServletRequest request, Throwable e) {
        // 用 requestId 代替
        errorLog.setTraceId(RequestIdContext.getRequestId());
        // 处理用户信息
    ...
    private CommonResult<?> handleTableNotExists(Throwable ex) {
        ...
        //     log.error("[报表模块 drm-module-report - 表结构未导入]");
        //     return CommonResult.error(NOT_IMPLEMENTED.getCode(),
        //             "[报表模块 drm-module-report - 表结构未导入]");
        // }
        // // 2. 工作流
        // if (message.contains("bpm_")) {
        //     log.error("[工作流模块 drm-module-bpm - 表结构未导入]");
        //     return CommonResult.error(NOT_IMPLEMENTED.getCode(),
        //             "[工作流模块 drm-module-bpm - 表结构未导入]");
        // }
        // // 3. 微信公众号
        // if (message.contains("mp_")) {
        //     log.error("[微信公众号 drm-module-mp - 表结构未导入]");
        //     return CommonResult.error(NOT_IMPLEMENTED.getCode(),
        //             "[微信公众号 drm-module-mp - 表结构未导入]");
        // }
        // // 4. 商城系统
        // if (StrUtil.containsAny(message, "product_", "promotion_", "trade_")) {
        //     log.error("[商城系统 drm-module-mall - 已禁用]");
        //     return CommonResult.error(NOT_IMPLEMENTED.getCode(),
        //             "[商城系统 drm-module-mall - 已禁用]");
        // }
        // // 5. ERP 系统
        // if (message.contains("erp_")) {
        //     log.error("[ERP 系统 drm-module-erp - 表结构未导入]");
        //     return CommonResult.error(NOT_IMPLEMENTED.getCode(),
        //             "[ERP 系统 drm-module-erp - 表结构未导入]");
        // }
        // // 6. CRM 系统
        // if (message.contains("crm_")) {
        //     log.error("[CRM 系统 drm-module-crm - 表结构未导入]");
        //     return CommonResult.error(NOT_IMPLEMENTED.getCode(),
        //             "[CRM 系统 drm-module-crm - 表结构未导入]");
        // }
        // // 7. 支付平台
        // if (message.contains("pay_")) {
        //     log.error("[支付模块 drm-module-pay - 表结构未导入]");
        //     return CommonResult.error(NOT_IMPLEMENTED.getCode(),
        //             "[支付模块 drm-module-pay - 表结构未导入]");
        // }
        // // 8. AI 大模型
        // if (message.contains("ai_")) {
        //     log.error("[AI 大模型 drm-module-ai - 表结构未导入]");
        //     return CommonResult.error(NOT_IMPLEMENTED.getCode(),
        //             "[AI 大模型 drm-module-ai - 表结构未导入]");
        // }
        return null;
    }
