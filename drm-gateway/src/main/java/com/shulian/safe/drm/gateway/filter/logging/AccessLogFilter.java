package com.shulian.safe.drm.gateway.filter.logging;
...
import com.shulian.safe.drm.gateway.util.SecurityFrameworkUtils;
import com.shulian.safe.drm.gateway.util.WebFrameworkUtils;
 ...
 * 网关的访问日志过滤器
 *
 * 从功能上，它类似 drm-spring-boot-starter-web 的 ApiAccessLogFilter 过滤器
 *
...
public class AccessLogFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        // reactor.netty.http.server.AccessLogHandler
        // reactor.netty.http.server.logging.AccessLogHandlerH1
        // reactor.netty.http.server.logging.AccessLogHandlerH2

        // 添加 RequestId
        String requestId = request.getHeaders().getFirst(GlobalConstants.X_REQUEST_ID);
        if (StringUtils.isBlank(requestId)) {
            requestId = IdUtil.fastUUID();
            ServerHttpRequest newRequest = request.mutate()
                    .header(GlobalConstants.X_REQUEST_ID, requestId)
                    .build();
        ...
        // TODO traceId
        // gatewayLog.setTraceId(requestId);
        ...
        Mono<Void> voidMono = filterWithoutRequestBody(exchange, chain, gatewayLog);
        return voidMono;
    }
    ...
    private void writeAccessLog(AccessLog gatewayLog) {
        ...
        values.put("requestId", gatewayLog.getRequestId());
        values.put("userId", gatewayLog.getUserId());
        values.put("userType", gatewayLog.getUserType());
        values.put("userName", gatewayLog.getUserName());
        ...
        // log.info("[writeAccessLog][网关日志]：{}", JsonUtils.toJsonString(values));
    }
        ...
        return new ServerHttpResponseDecorator(response) {
            ...
            public Mono<Void> writeWith(Publisher<? extends DataBuffer> body) {
                if (body instanceof Flux) {
                    ...
                    gatewayLog.setUserName(SecurityFrameworkUtils.getLoginUseName(exchange));
