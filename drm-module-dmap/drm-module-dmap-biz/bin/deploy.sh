...
DATE=$(date +%Y%m%d%H%M)
# 基础路径
BASE_PATH=/work/projects/drm-dmap
# 编译后 jar 的地址。部署时，Jenkins 会上传 jar 包到该目录下
SOURCE_PATH=$BASE_PATH/build
# 服务名称。同时约定部署服务的 jar 包名字也为它。
SERVER_NAME=drm-module-dmap-biz
...
#远程调试
#JAVA_AGENT="-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=50055"
...
function start() {
  ...
  # 开始启动
  setsid java -server $JAVA_OPS $JAVA_AGENT -jar $BASE_PATH/$SERVER_NAME.jar --spring.profiles.active=$PROFILES_ACTIVE >/dev/null 2>&1 &

  echo "[start] 启动命令 -> java -server $JAVA_OPS $JAVA_AGENT -jar $BASE_PATH/$SERVER_NAME.jar --spring.profiles.active=$PROFILES_ACTIVE >/dev/null 2>&1 "
