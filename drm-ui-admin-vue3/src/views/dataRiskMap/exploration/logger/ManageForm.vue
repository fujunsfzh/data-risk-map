<template>
  <Dialog v-model="dialogVisible" title="添加" width="800">
    <el-form
      ref="formRef"
    ...
    >
      ...
      <el-form-item label="连接参数" prop="extParams">
...
const open = (row?: any) => {
  ...
  if (row) {
    ...
    try {
      formData.value.dsType = row.dsType
      formData.value.dbHost = row.dsUrl
      formData.value.dbPort = row.dsPort
    } finally {
